...
function findHazards(text){
  ...
  for(const h of HAZARDS){
    ...
    if(re.test(text)) set.add(h);
  }

  // Also capture +/- variants for the key phenomena
  const generic = text.match(/\b(\+|-)?(RA|SN|TS|FG|DZ)\b/g);
  if(generic){
...
async function main(){
  ...
  const stations = icaos.map(icao => {
    ...
    const metarHtml = metar ? highlightRaw(metar, metarVis.tokensToHighlight) : null;
    const tafHtml = taf ? highlightRaw(taf, tafVis.tokensToHighlight) : null;
